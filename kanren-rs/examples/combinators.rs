...
fn bind2(e: Var, comb: Var, a: Var, b: Var, rem: Var, v: Var, v_: Var) -> Goal {
    fresh(move |vn| cond([
        ...
        vec![eq(e, list!(comb)), eq(rem, NULL), get_var(a, v, vn), get_var(b, vn, v_)],
    ]))
});

goal!(
fn bind3(e: Var, comb: Var, a: Var, b: Var, c: Var, rem: Var, v: Var, v_: Var) -> Goal {
    fresh(move |v0, v1| cond([
        vec![eq(e, list!(comb, a, b, c, . rem)), eq(v, v_)],
        vec![eq(e, list!(comb, a, b)), eq(rem, NULL), get_var(c, v, v_)],
        vec![eq(e, list!(comb, a)), eq(rem, NULL), get_var(b, v, v0), get_var(c, v0, v_)],
        vec![eq(e, list!(comb)), eq(rem, NULL), get_var(a, v, v0), get_var(b, v0, v1), get_var(c, v1, v_)],
    ]))
});

goal!(
fn apply(e: Var, e_: Var, v: Var, v_: Var) -> Goal {
    fresh(move |a, b, c, rem| cond([
        vec![bind1(e, "I", a, rem, v, v_), eq(e_, cons(a, rem))],
        vec![bind2(e, "K", a, b, rem, v, v_), eq(e_, cons(a, rem))],
        vec![bind3(e, "S", a, b, c, rem, v, v_), eq(e_, list!(a, c, list!(b, c), . rem))],
    ]))
});
...
fn var(e: Var, e_: Var, t: Var, t_: Var, v: Var) -> Goal {
    fresh(move |x| cond([
        vec![eq(e, cons(x, e_)), eq(t_, cons(x, t)), list::contains(v, x)],
    ]))
});
...
fn eval_inner(e: Var, v: Var, v_: Var, t: Var, t_: Var) -> Goal {
    cond([
        ...
        // Apply combinator
        vec![fresh(move |te, tv| all([
            apply(e, te, v, tv), jield(move || eval_inner(te, tv, v_, t, t_))
        ]))],
        // If var, no further reduction is possible, move head to term
        vec![fresh(move |te, tt| all([
            var(e, te, t, tt, v), jield(move || eval_inner(te, v, v_, tt, t_))
        ]))],
        // Unwrap head if term is empty
        vec![fresh(move |head, tail, te| all([
            eq(t, NULL), eq(e, cons(head, tail)), list::at_least_two(head),
            list::append(head, tail, te),
            jield(move || eval_inner(te, v, v_, t, t_))
        ]))],
        // Reduce non-head sub expressions
        vec![fresh(move |sub, res, tail, tt| all([
            list::not_empty(t), eq(e, cons(sub, tail)), list::at_least_two(sub),
            cond([
                vec![eq(tt, cons(res, t)), list::at_least_two(res)],
                vec![fresh(move |single| all([ eq(tt, cons(single, t)), eq(res, list!(single))]))],
            ]),
...
fn combs(expr: Var) -> Goal {
    cond([
        [eq(expr, NULL)],
        [fresh(move |comb, tail| all([
            eq(expr, cons(comb, tail)), is_comb(comb), jield(move || combs(tail))
        ]))],
        [fresh(move |l, tail| all([
            eq(expr, cons(l, tail)), list::at_least_two(l), jield(move || combs(l)), jield(move || combs(tail))
        ]))]
    ])
});
...
fn test_apply(input: &str, expected: &str) {
    ...
    let result = run(1, |v, e|
        apply(&expr, e, NULL, v)
    );
    let result = AsScheme(result).to_string();
    let (mark, equal) = if result == expected { (" ", "==") } else { ("*", "!=") };
...
fn main() {
    println!("{} {:24} {:24} {} {}", " ", "Input", "Expected", "  ", "Result");
    test_apply("I", "(((a) (a)))");
    test_apply("Ix", "((() (x)))");
    test_apply("Ixy", "((() (x y)))");
    test_apply("K", "(((b a) (a)))");
    ...
    while let Some(state) = result.next() {
        ...
        println!("{:8} {}", fmt_list(&vars[0]), fmt_lambda(&vars[1], &vars[2]));
    }
}